...
def init_agents(n_m_users, n_items):
...
def crossover(agents, cur_generation):
...
def mutation(agents):
    for agent in agents:
        if np.random.rand() < MUTATE_USER_PROB:
            agent.gnome = np.apply_along_axis(mutate_1d_array, 0, agent.gnome)
...
def print_stats(agents, cur_generation):
...
POS_RATIO = 0.001 # Ratio pos/ neg ratio  one percent from each user
...
def main():
    ...
    agents = init_agents(N_FAKE_USERS, N_ITEMS)
    print('created n_agents', len(agents))
    print_stats(agents, 0)
    for cur_generation in range(1, N_GENERATIONS):
        agents = fitness(agents)
        if cur_generation % 50 == 0:
            print_stats(agents , cur_generation)

        agents = selection(agents)
        agents = crossover(agents, cur_generation)
