...
def get_generator(shape, gf_dim=64, b_w = True): # Dimension of gen filters in first conv layer. [64]
    image_size = 64
    s16 = image_size // 16
...
def get_discriminator(shape, df_dim=64, b_w = True): # Dimension of discrim filters in first conv layer. [64]
    # w_init = tf.glorot_normal_initializer()
    w_init = tf.random_normal_initializer(stddev=0.02)
    gamma_init = tf.random_normal_initializer(1., 0.02)
